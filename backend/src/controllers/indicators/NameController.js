...
class NameController {
    ...
    async save(value){
        ...
        const item = await this._databases.query('INSERT INTO indi_name (id,name_th, name_en) VALUES (?,?,?)',[
    ...
    async update(id, value) {  
        ...
        await this._databases.query("UPDATE indi_name SET name_th=?, name_en=? WHERE id=?",[
            value['name_th'],
            value['name_en'],
            id
        ])
    ...
    async delete(id){
        ...
        console.log(errors);       
        if (errors) throw { errors };
        return await this._database.query('DELETE FROM indi_name where id=?', [id]);
    }
}
